class FileUploader {
    ...
    async uploadFile(file, fileId = null) {
        ...
        try {
            ...
            if (this.onProgress) this.onProgress(finalFileId, uploadInfo);
            if (this.onComplete) this.onComplete(finalFileId, file);
    ...
    async sendFileChunks(file, fileId, totalChunks) {
        ...
        while (currentIndex < totalChunks) {
            ...
            const now = Date.now();
            uploadInfo.duration = (now - uploadInfo.startTime) / 1000;
            ...
            if (this.onProgress) this.onProgress(fileId, uploadInfo);

            await this.delay(1);

            if (chunkQueue.length > 0) chunkQueue.shift();
        }
    }
